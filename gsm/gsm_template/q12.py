### Code adapted from https://github.com/yifanzhang-pro/syntax-semantics/blob/main/template-gsm/gsm-0012-1.py

# Origin problem: {"question": "Randy has 60 mango trees on his farm. 
# He also has 5 less than half as many coconut trees as mango trees. H
# ow many trees does Randy have in all on his farm?", 
...
def generate_problem_and_solution_code():
    ...
    problem_statement = f"{name} has {initial_amount} {item1} at their farm. "
    problem_statement += f"They also have {adjustment} less than half as many {item2} as {item1}. "
    problem_statement += "How many items does " + name + " have on their farm?"
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q12--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 12, "idx": i}) + '\n')