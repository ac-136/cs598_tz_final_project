### Code adapted from: https://github.com/yifanzhang-pro/syntax-semantics/blob/main/template-gsm/gsm-0001-1.py
...
def generate_problem_and_solution_code():
    ...
    problem_statement = f"{name} earns ${hourly_rate} an hour for {activity}. "
    problem_statement += f"Yesterday, they just did {working_minutes} minutes of {activity}. How much did {name} earn?"
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q1--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 1, "idx": i}) + '\n')