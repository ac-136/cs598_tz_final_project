### Code adapted from https://github.com/yifanzhang-pro/syntax-semantics/blob/main/template-gsm/gsm-0013-1.py

# Origin problem: {"question": "Jasper will serve charcuterie at his dinner party. 
# He buys 2 pounds of cheddar cheese for $10, a pound of cream cheese that cost half the price of the cheddar cheese, 
# and a pack of cold cuts that cost twice the price of the cheddar cheese. How much does he spend on the ingredients?", 
...
def generate_problem_and_solution_code():
    ...
    problem_statement = f"{name} will serve charcuterie at their dinner party. "
    problem_statement += f"They buy 2 pounds of {item1} for ${initial_cost}, a pound of {item2} that cost half the price of the {item1}, "
    problem_statement += f"and a pack of {item3} that cost twice the price of the {item1}. "
    problem_statement += f"How much does {name} spend on the ingredients?"
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q13--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 13, "idx": i}) + '\n')