### Code adapted from https://github.com/yifanzhang-pro/syntax-semantics/blob/main/template-gsm/gsm-0008-1.py

# Origin problem: {"question": "Alexis is applying for a new job and bought a new set of business clothes to wear to the interview. 
# She went to a department store with a budget of $200 and spent $30 on a button-up shirt, $46 on suit pants, $38 on a suit coat, $11 on socks, and $18 on a belt. 
# She also purchased a pair of shoes, but lost the receipt for them. She has $16 left from her budget. How much did Alexis pay for the shoes?", 
...
def generate_problem_and_solution_code():
    ...
    initial_budget = 200
    known_expenses = [30, 46, 38, 11, 18, 16]

    activity = "shopping"
    item = "test"

    # Construct problem statement with specific details
    problem_statement = f"{name} is applying for a new job and bought a new set of business clothes to wear to the interview. "
    problem_statement += f"They went to a department store with a budget of ${initial_budget} and spent ${known_expenses[0]} on a button-up shirt, ${known_expenses[1]} on suit pants, ${known_expenses[2]} on a suit coat, ${known_expenses[3]} on socks, and ${known_expenses[4]} on a belt."
    problem_statement += f"They also purcahsed a pair of shoes, but lost the receipt for them. They have ${known_expenses[-1]} left from their budget. "
    problem_statement += f"How much did {name} pay for the shoes?"
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q8--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 8, "idx": i}) + '\n')