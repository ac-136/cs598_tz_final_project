...
def generate_problem_and_solution_code():
    ...
    problem_statement = f"{name} creates a media empire. They create a movie for ${initial_investment}. "
    problem_statement += f"Each DVD costs ${production_cost} to make. "
    problem_statement += f"{name} sells it for {sales_multiplier} times that much. "
    problem_statement += f"They sell {daily_sales} movies a day for {days_per_week} days a week. "
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q15--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 15, "idx": i}) + '\n')