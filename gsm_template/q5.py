### Code adapted from https://github.com/yifanzhang-pro/syntax-semantics/blob/main/template-gsm/gsm-0005-1.py

# Origin problem: {"question": "Mark has a garden with flowers. He planted plants of three different colors in it. Ten of them are yellow, and there are 80% more of those in purple. There are only 25% as many green flowers as there are yellow and purple flowers. How many flowers does Mark have in his garden?", 
...
def generate_problem_and_solution_code():
    ...
    problem_statement = f"{name} has a garden with flowers. "
    problem_statement += f"They planted plants of three different colors in it. {initial_amount} of them are yellow, and there are {int(extra_ratio*100)}% more of those in purple. "
    problem_statement += f"There are only {int(percentage * 100)}% as many green flowers as there are yellow and purple flowers. "
    problem_statement += f"How many flowers does {name} have in their garden?"
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q5--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 5, "idx": i}) + '\n')