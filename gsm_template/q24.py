### Code adapted from https://github.com/yifanzhang-pro/syntax-semantics/blob/main/template-gsm/gsm-0024-1.py


# Origin problem: {"question": "Mary does her grocery shopping on Saturday. 
# She does her shopping only at a specific store where she is allowed a credit of $100, which must be paid in full before her next shopping trip. 
# That week she spent the full credit limit and paid $15 of it on Tuesday and $23 of it on Thursday. 
# How much credit will Mary need to pay before her next shopping trip?", 
...
def generate_problem_and_solution_code():
    ...
    problem_statement = f"{name} does their grocery shopping on Saturday. "
    problem_statement += f"They do their shopping only at a specific store where they are allowed a credit of ${initial_amount}, which must be paid in full before their next shopping trip. "
    problem_statement += f"That week they spent the full credit limit and paid ${subtraction1} of it on {day1} and ${subtraction2} of it on {day2}. "
    problem_statement += f"How much {item} will {name} need to pay before their next shopping trip?"
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q24--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 24, "idx": i}) + '\n')