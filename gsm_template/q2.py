### Code Adapted from: https://github.com/yifanzhang-pro/syntax-semantics/blob/main/template-gsm/gsm-0002-1.py
...
def generate_problem_and_solution_code():
    ...
    item = "wallet"
    target_amount = 100
    initial_savings = target_amount / 2
    parent_contribution = 15
    # item = random.choice(items)

    # Construct problem statement with specific details
    problem_statement = f"{name} is saving money for a new {item} which costs ${target_amount}. "
    problem_statement += f"{name} has only half of the money they need. "
    problem_statement += f"Their parents decided to give them ${parent_contribution} for that purpose, "
    problem_statement += f"and their grandparents twice as much as their parents. "
...
{friend_contribution} = {parent_contribution}
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q2--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 2, "idx": i}) + '\n')