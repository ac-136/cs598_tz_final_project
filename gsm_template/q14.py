### Code adapted from https://github.com/yifanzhang-pro/syntax-semantics/blob/main/template-gsm/gsm-0014-1.py

# Origin problem: {"question": "Joy can read 8 pages of a book in 20 minutes. 
...
def generate_problem_and_solution_code():
    ...
    problem_statement = f"{names} can read {per_unit} {item} of a book in {time} minutes. "
    problem_statement += f"How many hours will it take them to read {total_amount} {item}?"
    ...
    solution_code = f"""# Number of {item} read by {names} per hour
{per_day_var} = {per_unit} * (60/{time})
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q14--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 14, "idx": i}) + '\n')