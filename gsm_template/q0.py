### Code adapted from https://github.com/yifanzhang-pro/syntax-semantics/blob/main/template-gsm/gsm-0000-1.py
...
def generate_problem_and_solution_code():
    ...
    problem_statement = f"{name} sold {item} to {initial_amount} of their friends in April, and then they sold {subsequent_ratio*100:.0f}% as many clips in May."
    problem_statement += f"How many {item} did {name} sell altogether in April and May?"
...
# Calculating the amount of {item} sold 
...
if __name__ == "__main__":
    ...
    # output jsonl file
    with open(f'./output/q0--NUM{NUM_PROBLEMS}.jsonl', 'w') as f:
        for i in range(NUM_PROBLEMS):
            problem, result = generate_problem_and_solution_code()
            # Write problem to file
            f.write(json.dumps({"problem": problem, "result": str(result), "template_name": 0, "idx": i}) + '\n')